    ...
    def post(self, request,  format=None):
        ...
        try:
            ...
            if user.is_authenticated:
                ...
                return response.Response({'error': 'Please authenticate user'}, status.HTTP_401_UNAUTHORIZED)
        except BaseException:
            return response.Response({'error':'Please, fill in the required fields and  image should be in jpeg format only '},
                                status=status.HTTP_400_BAD_REQUEST)

            
# for changeing the plan
class ChangePlan(APIView):
    def post(self, request, format=None):
        try:
            ...
            return response.Response({'error':'Please select a plan '},
                                status=status.HTTP_400_BAD_REQUEST)


    
# for aunthenticated users
